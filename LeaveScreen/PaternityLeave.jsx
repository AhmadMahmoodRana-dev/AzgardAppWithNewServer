...
const PaternityLeave = () => {
  ...
  const data = [{label: 'Paternity Leave', value: '19'}];
  ...
  const handleFileUpload = async () => {
    ...
    try {
      ...
      if (xStatus === '200') {
        ...
        const selectedLabel =
          data.find(item => item.value === selectedValue)?.label || 'Unknown';
        ...
        Alert.alert(
          'Error',
          '❌  A leave already exists for the selected date(s).',
        );
      } else {
        ...
        <Text style={[styles.cell, styles.greenCell]}>{item.TOTAL_BALANCE_VALUE}</Text>
            ...
            <View style={styles.containerdropdown}>
              ...
              {loading ? (
                ...
                leavHistory
                  .filter(item => item.LEAVE_TYPE === 'Paternity Leave')
                  .map(item => (
                    <View
