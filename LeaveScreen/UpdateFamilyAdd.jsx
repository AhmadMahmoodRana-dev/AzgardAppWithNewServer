...
const UpdateFamilyAdd = () => {
  ...
  const formatCnic = text => {
    ...
    if (cleanText.length > 12) {
      formattedText = `${cleanText.slice(0, 5)}-${cleanText.slice(
        5,
        12,
      )}-${cleanText.slice(12, 13)}`;
    }
  ...
  const fetchLeaveRequests = async () => {
    try {
      setLoading(true);
      const response = await fetch(`${BASEURL}/ords/api/emp_update/get?p_user_id=${global.xx_user_id}`);
  ...
  const handleFileUpload = async () => {
    ...
    try {
      const response = await axios.post(
        `${BASEURL}/ords/api/family/insert`,
        formData,
        {
          headers: {
              ...
              {loading ? (
                ...
                leavHistory.map(item => (
                  <View
                    ...
                    key={item.EMP_FAMILY_ID.toString()}>
