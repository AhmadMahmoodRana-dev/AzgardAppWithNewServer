...
const ShowSalaryChangesApprovalEntry = () => {
  ...
  const fetchLeaveRequests = async () => {
    try {
      ...
      const response = await fetch(
        `${BASEURL}/ords/api/Get_Promotion_Approval/get?X_EMP_ID=${global.xx_emp_id}`,
      );
      const data = await response.json();
      setLeaveRequests(data.Promotion_approval);
    } catch (error) {
  ...
  const approveRequest = async (id, TYPE, empId, leaveType, noOfDays) => {
    try {
      ...
      const response = await axios.put(
        `${BASEURL}/ords/api/Put_Promotion_Approval/UPDATE`,
        {}, 
        {
          params: {
            p_promotion_id: id,
  ...
  const renderItem = ({item}) => (
    <View style={styles.card}>
      ...
      <Text style={styles.text}>Current Designation: {item?.DESIGNATION}</Text>
      <Text style={styles.text}>Current Pay: {item?.CURRENT_PAY}</Text>
      <Text style={styles.text}>Proposed Designation: {item?.PROOPOSED_DESIG}</Text>
      <Text style={styles.text}>Proposed Pay: {item?.PROPOSED_SAL}</Text>
      <Text style={styles.text}>Reason: {item?.REASON_FOR_PROMO}</Text>
...
export default ShowSalaryChangesApprovalEntry;