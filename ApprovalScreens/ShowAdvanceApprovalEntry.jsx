...
const ShowAdvanceApprovalEntry = () => {
  ...
  const fetchLeaveRequests = async () => {
    try {
      ...
      const response = await fetch(
        `${BASEURL}/ords/api/get_Advance_Approval/GET?X_EMP_ID=${global.xx_emp_id}`,
      );
      const data = await response.json();
      setLeaveRequests(data.Advance_approval);
    } catch (error) {
  ...
  const approveRequest = async (id, TYPE, empId, leaveType, noOfDays) => {
    try {
      ...
      const response = await axios.put(
        `${BASEURL}/ords/api/Put_Advance_Approval/UPDATE`,
        {}, 
        {
          params: {
            p_advance_id: id,
...
export default ShowAdvanceApprovalEntry;